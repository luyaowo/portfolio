...
输入一组关键词，就能生成一张奇妙的画面。我也曾以为，这就是 prompt 的全部。

再后来开始更多地用大语言模型处理文档和辅助工作，在使用中对 prompt 的概念逐渐弱化，把对话式 AI 想成电脑对面坐着一个无所不知的人，觉得措辞够讨喜，它就会愿意好好干。
...
从最基础的生成机制看，大语言模型会根据当前上下文，逐个预测接下来最可能出现的 Token。我们输入的每个字，都会改变后续内容出现的概率。打个不太严谨但好理解的比方，它像是在不断调整方向，把接下来要生成的文字推向某一类文本模式。
...
为什么会出现这种情况？因为在训练数据中，“科普文章”“普通读者”“技术解释”等信息，经常和某些表达方式一起出现。当模型接收到这些条件时，后续内容的概率分布会发生变化，更容易生成与这个场景相符的文本。这里说的“进入某种文本模式”只是一个方便理解的比方，并不是模型真的进入了训练数据里的某个区域。
...
模型就知道：
这篇文章的目的不是介绍 AI 行业，也不是写技术论文，而是帮助普通读者建立基础认知。
因此它会更倾向于从读者熟悉的问题切入，减少专业术语，使用生活化例子解释概念，并按照科普文章的方式组织内容。
...
“科技科普作者”会影响模型的表达方式：它会更倾向于使用通俗的语言、生活化的例子，而不是直接堆砌技术概念。

不过，角色并不是越厉害越好。很多人喜欢写：
...
这种通过提供示例，让模型理解任务要求的方法，就叫 Few-shot（少样本）。
其中：
...
不过，Few-shot 并不是简单地“多给几个例子”。示例本身的质量非常重要。如果示例：和目标任务不一致；风格前后矛盾；存在错误信息。模型可能会学习到错误方向。同时，示例数量也不是越多越好。
过多的示例会占用上下文空间，让模型需要处理更多无关信息。

Few-shot 的关键不在示例数量，而在示例是否清晰、一致，并且贴近当前任务。当一个要求很难用语言描述时，与其继续堆“自然”“高级”之类的形容词，不如直接给模型看你认为什么样才算好。

### 3.2 CoT、任务拆解和 Prompt Chaining

有些任务的问题，不是模型不知道怎么回答，而是任务本身包含多个步骤。

例如：

> 分析这份用户反馈，判断产品主要问题，并给出优化建议。

这个任务实际上包含整理反馈信息、发现共性问题、判断问题优先级、分析产生原因和制定优化方向等多个动作。

如果直接要求模型完成，它需要自己决定分析路径。对于简单问题，这没有影响；但对于复杂任务，不同的处理方式可能会得到不同结果。

这时候，可以让任务按照明确流程展开：

> 请分析这份用户反馈。先归纳用户主要问题，再判断问题优先级，分析可能原因，最后提出优化建议。

也可以进一步要求模型：

> 如果任务缺少合适的分析框架，请先建立分析思路，再完成分析。

这种写法更准确地说，属于**任务拆解**：我们提前把复杂任务分成几个步骤，让模型少猜一点处理顺序。

Chain of Thought（CoT，思维链）原本指的是让模型在回答之前生成一系列中间推理步骤。它关注的是模型如何一步步推到答案，任务拆解关注的是我们如何提前安排处理流程。两者经常一起出现，所以很容易被混为一谈，但并不是一回事。

现在不少模型已经带有专门的推理能力，并不需要每次都要求它展示完整的思考过程。更实用的要求是：先分析问题，再给出结论和关键依据。这样既能帮助模型处理复杂任务，也方便我们检查它最后的判断从哪里来。

如果任务再复杂一些，还可以把几个步骤拆成多次模型调用：先整理用户反馈，再把整理结果交给下一次调用判断优先级，最后根据前面的结果生成方案。

这叫 **Prompt Chaining（提示链）**。它和在一条 Prompt 里列出步骤不同，因为每一步都可以单独检查、修改，甚至换一个模型重新处理。

### 3.3 什么时候需要增强 Prompt

Few-shot、任务拆解和 CoT 都是在基础 Prompt 之外，进一步提高结果稳定性的方法。但它们并不是每次都需要使用。

对于“把下面这句话翻译成英文”这样的简单任务，目标已经很明确，模型不需要额外示例或复杂步骤。对于“总结这篇文章的三个核心观点”这样有明确标准的任务，只需要说明任务和输出要求即可。

当结果标准很难用语言描述时，可以补充 Few-shot 示例。任务包含多个分析环节时，可以先拆解处理步骤；一条 Prompt 装不下，或者每一步都需要单独检查时，再使用 Prompt Chaining。

至于 CoT，更适合需要推理、判断和计算的任务，而不是所有复杂任务都要强行加一句“请一步步思考”。

所以，Prompt 优化并不是不断增加更多内容。一个好的原则是：先用最简单的 Prompt 解决问题；当结果不稳定时，再补充示例、步骤和其他限制。基础 Prompt 解决的是：让模型知道要做什么。增强方法解决的是：让模型更稳定地按照预期完成。
